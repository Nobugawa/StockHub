...
import { ChevronDown, ChevronRight, Eye, EyeOff, LogOut, Plus, RefreshCw, Search } from 'lucide-react'
...
function App(){
 const [session,setSession]=useState(null),[stocks,setStocks]=useState([]),[screens,setScreens]=useState([]),[themes,setThemes]=useState([]),[search,setSearch]=useState(''),[selected,setSelected]=useState(null),[selectedTheme,setSelectedTheme]=useState(null),[showAdd,setShowAdd]=useState(false),[industriesOpen,setIndustriesOpen]=useState(true),[candidatesOpen,setCandidatesOpen]=useState(true),[loading,setLoading]=useState(true)
 ...
 const load=async()=>{setLoading(true);const [{data:s,error:e1},{data:sc,error:e2},{data:th,error:e3}]=await Promise.all([supabase.from('stock_dashboard').select('*').order('ticker'),supabase.from('screen_definitions').select('*').eq('active',true).order('name'),supabase.from('industry_themes').select('*,industry_memberships(role,maturity,priority,notes,stocks(id,ticker,company_name)),industry_research_snapshots(*)').eq('status','active').order('name')]);if(e1)console.error(e1);if(e2)console.error(e2);if(e3)console.error(e3);setStocks(s||[]);setScreens(sc||[]);setThemes(th||[]);setLoading(false)}
 ...
 <section className="candidate-section"><button className="candidate-toggle" onClick={()=>setIndustriesOpen(v=>!v)} aria-expanded={industriesOpen}>{industriesOpen?<ChevronDown size={18}/>:<ChevronRight size={18}/>} Industry & Sector Research <span className="pill">{themes.length}</span></button>
 {selectedTheme&&<IndustryDetail theme={selectedTheme} onClose={()=>setSelectedTheme(null)}/>}
 {industriesOpen&&<div className="industry-grid">{themes.map(t=><button key={t.id} className="industry-card" onClick={()=>{setSelectedTheme(t);window.scrollTo({top:0,behavior:'smooth'})}}><div className="muted" style={{fontSize:11}}>{t.sector||'Sector'}</div><h3>{t.name}</h3><p>{t.thesis}</p><div>{(t.industry_memberships||[]).sort((a,b)=>(a.priority==='focus'?-1:1)).map(m=><span className={'pill '+(m.priority==='focus'?'good':'')} key={m.stocks?.ticker||m.id}>{m.stocks?.ticker||'—'}</span>)}</div><div className="muted" style={{fontSize:11,marginTop:8}}>Updated {t.as_of_date}</div></button>)}</div>}</section>
  <section className="candidate-section"><button className="candidate-toggle" onClick={()=>setCandidatesOpen(v=>!v)} aria-expanded={candidatesOpen}>{candidatesOpen?<ChevronDown size={18}/>:<ChevronRight size={18}/>} Candidates <span className="pill">{filtered.length}</span></button>
 {selected&&<StockDetail stock={selected} onClose={()=>setSelected(null)} onSaved={()=>{load();setSelected(null)}}/>}
 {candidatesOpen&&<div className="table-wrap"><table className="table"><thead><tr><th>Ticker</th><th>Company</th><th>Price</th><th>Market cap</th><th>Revenue growth</th><th>Gross margin</th><th>Cash / debt</th><th>Insiders</th><th>Technical</th><th>Research</th></tr></thead><tbody>{filtered.map(s=><tr key={s.id} onClick={()=>{setSelected(s);window.scrollTo({top:0,behavior:'smooth'})}} style={{cursor:'pointer'}}><td className="ticker">{s.ticker}</td><td>{s.company_name}<div className="muted" style={{fontSize:11}}>{s.sector||''}</div></td><td>{s.last_price??'—'}</td><td>{money(s.market_cap)}</td><td>{pct(s.revenue_growth_yoy)}</td><td>{pct(s.gross_margin)}</td><td>{money(s.cash_and_equivalents)} / {money(s.total_debt)}</td><td>{s.significant_net_insider_buying?<span className="pill good">NET BUYING</span>:<span className="pill">—</span>}</td><td>{s.latest_technical_summary||'—'}</td><td>{s.last_researched_at?new Date(s.last_researched_at).toLocaleDateString():'—'}</td></tr>)}</tbody></table></div>}</section>
...
function IndustryDetail({theme,onClose}){
 const r=(theme.industry_research_snapshots||[]).slice().sort((a,b)=>String(b.as_of_date).localeCompare(String(a.as_of_date)))[0]
 const members=(theme.industry_memberships||[]).slice().sort((a,b)=>(a.priority==='focus'?0:1)-(b.priority==='focus'?0:1))
 return <div className="detail card"><div style={{display:'flex',justifyContent:'space-between',gap:10}}><div><h2 style={{margin:'0 0 5px'}}>{theme.name}</h2><div className="muted">{theme.sector} • updated {theme.as_of_date}</div></div><button className="btn" onClick={onClose}>Close</button></div>
 <div className="industry-detail-grid"><div><h3>Sector thesis</h3><div className="snapshot">{theme.thesis}</div></div><div><h3>Value chain</h3><div className="snapshot">{theme.value_chain}</div></div><div><h3>What to track</h3><div className="snapshot">{theme.key_metrics}</div></div><div><h3>Key risks</h3><div className="snapshot">{theme.key_risks}</div></div></div>
 {r&&<><div className="card" style={{marginTop:12}}><b>Latest sector research • {r.as_of_date}</b><p>{r.summary}</p><p><b>Demand signals:</b> {r.demand_signals}</p><p><b>Catalysts:</b> {r.catalysts}</p><p><b>Risks:</b> {r.risks}</p></div><h3>Surrounding companies</h3><div className="table-wrap"><table className="table"><thead><tr><th>Ticker</th><th>Company</th><th>Role</th></tr></thead><tbody>{(r.surrounding_companies||[]).map((x,i)=><tr key={i}><td className="ticker">{x.ticker||'Private'}</td><td>{x.name}</td><td>{x.role}</td></tr>)}</tbody></table></div></>}
 <h3 style={{marginTop:16}}>StockHub names in this theme</h3><div className="table-wrap"><table className="table"><thead><tr><th>Ticker</th><th>Company</th><th>Role</th><th>Stage</th><th>Priority</th><th>What to watch</th></tr></thead><tbody>{members.map((m,i)=><tr key={i}><td className="ticker">{m.stocks?.ticker}</td><td>{m.stocks?.company_name}</td><td>{m.role}</td><td>{m.maturity}</td><td>{m.priority}</td><td>{m.notes}</td></tr>)}</tbody></table></div></div>
}
...
function StockDetail({stock,onClose}){const [tab,setTab]=useState('scorecard'),[scorecards,setScorecards]=useState([]),[snapshots,setSnapshots]=useState([]),[fundamentals,setFundamentals]=useState([]),[technicals,setTechnicals]=useState([]),[news,setNews]=useState([]),[insiders,setInsiders]=useState([]),[institutions,setInstitutions]=useState([])
 useEffect(()=>{Promise.all([supabase.from('research_scorecards').select('*').eq('stock_id',stock.id).order('as_of_date',{ascending:false}),supabase.from('research_snapshots').select('*').eq('stock_id',stock.id).order('as_of_date',{ascending:false}),supabase.from('fundamental_snapshots').select('*').eq('stock_id',stock.id).order('as_of_date',{ascending:false}),supabase.from('technical_snapshots').select('*,screen_definitions(name)').eq('stock_id',stock.id).order('as_of_date',{ascending:false}),supabase.from('news_items').select('*').eq('stock_id',stock.id).order('published_at',{ascending:false}),supabase.from('insider_activity').select('*').eq('stock_id',stock.id).order('transaction_date',{ascending:false}),supabase.from('institutional_activity').select('*').eq('stock_id',stock.id).order('as_of_date',{ascending:false})]).then(rs=>{setScorecards(rs[0].data||[]);setSnapshots(rs[1].data||[]);setFundamentals(rs[2].data||[]);setTechnicals(rs[3].data||[]);setNews(rs[4].data||[]);setInsiders(rs[5].data||[]);setInstitutions(rs[6].data||[])})},[stock.id])
 ...
 {tab==='fundamentals'&&<div>{fundamentals.map(x=><div className="card" key={x.id} style={{marginBottom:10}}><b>{x.as_of_date}</b><div className="snapshot">Revenue {money(x.revenue)} • Growth {pct(x.revenue_growth_yoy)} • Gross margin {pct(x.gross_margin)} • FCF {money(x.free_cash_flow)} • Cash {money(x.cash_and_equivalents)} • Debt {money(x.total_debt)}</div>{x.notes&&<p>{x.notes}</p>}</div>)}{!fundamentals.length&&<p className="muted">No fundamentals snapshots yet.</p>}</div>}
